...
class OpenAIClient:
    def __init__(self):
        ...
        if self.api_key and not self.api_key.startswith("your_") and len(self.api_key) > 5:
            try:
                ...
                logger.info(
                    f"OpenAI AsyncClient initialized with model: {self.model}")
            except Exception as e:
                logger.error(
                    f"Failed to initialize OpenAI client: {e}. OpenAI provider will be skipped.")
                self.client = None
        else:
            logger.warning(
                "OPENAI_API_KEY not set or invalid. OpenAI provider is unavailable.")
    ...
    async def generate_json(self, system_prompt: str, user_prompt: str, schema: Optional[Any] = None) -> Dict[str, Any]:
        ...
        if not self.client:
            raise RuntimeError(
                "OpenAI unavailable: API key missing or invalid")
        ...
        try:
            ...
            logger.info(
                f"Successfully received response from OpenAI. Finish reason: {finish_reason}. Content length: {len(content) if content else 0}")
            if not content or content.strip() in ("{}", ""):
                logger.error(
                    f"OpenAI returned empty or trivial response. Raw content: {repr(content[:500] if content else '')}")
                return {}
            ...
            if isinstance(result, dict) and len(result) == 1:
                ...
                if isinstance(only_val, (list, dict)) and not any(k in result for k in ["nodes", "edges", "services"]):
                    logger.info(
                        f"Unwrapping single-key wrapper response. Keys: {list(result.keys())}")
                    return only_val
            return result
    ...
    def _generate_mock_response(system_prompt: str, user_prompt: str) -> Dict[str, Any]:
        ...
        if any(k in system_prompt.lower() for k in ["requirementunderstandingagent", "requirement understanding", "requirementanalysisagent", "requirement analysis", "analyze requirements", "understand requirements"]):
            ...
            if is_banking:
                ...
                reqs.append(
                    "Deploy active WAF controllers to intercept transaction routes.")
            if is_streaming:
                reqs.append(
                    "Deploy CDN gateways for media streaming endpoints.")
                reqs.append(
                    "Implement Redis caching to optimize throughput latency.")
            if is_crud:
                reqs.append(
                    "Deploy a cost-efficient lightweight VM or App Service container.")

            projectName = req_data.get("projectName") or req_data.get(
                "project_name") or "Enterprise Stack"
            region = req_data.get("region") or "East US"
            resourceGroup = req_data.get("resourceGroup") or req_data.get(
                "resource_group") or "rg-production"
            vnetCIDR = req_data.get("vnetCIDR") or req_data.get(
                "vnet_cidr") or "10.0.0.0/16"
            computeType = req_data.get("computeType") or req_data.get(
                "compute_type") or req_data.get("application_type") or "AKS"
            databaseType = req_data.get("database_type") or req_data.get(
                "databaseType") or "PostgreSQL"
            app_description = req_data.get("app_description") or req_data.get(
                "appDescription") or user_prompt
        ...
        # B. Architecture Reasoning / Planning Mock
            ...
            class DummyRequirements:
                def __init__(self, data):
                    self.projectName = data.get("projectName") or data.get(
                        "project_name") or "Enterprise Stack"
                    self.region = data.get("region") or "East US"
                    self.resourceGroup = data.get("resourceGroup") or data.get(
                        "resource_group") or "rg-production"
                    self.vnetCIDR = data.get("vnetCIDR") or data.get(
                        "vnet_cidr") or "10.0.0.0/16"
                    self.computeType = data.get("computeType") or data.get(
                        "compute_type") or data.get("application_type") or "AKS"
                    self.database_type = data.get("database_type") or data.get(
                        "databaseType") or "PostgreSQL"
                    self.app_description = data.get(
                        "app_description") or data.get("appDescription") or ""
                    self.expected_users = data.get(
                        "expected_users") or "10k monthly"
            ...
            engine = InfrastructureReasoningEngine(
                cloud_provider=provider, requirements=dummy_req)
        ...
        # C. Security Optimization Mock
            ...
            findings = [
                {"severity": "Low", "description": "Redirect all public HTTP traffic to secure HTTPS channels.",
                    "remediation": "Set force_ssl = true."}
            ]
            score = 80

            if is_banking:
                findings.append({"severity": "High", "description": "Highly secure financial nodes require strict vault access controls.",
                                "remediation": "Inject keys into container app environments."})
                score = 98
            ...
            return {
                ...
                "compliance_checks": [
                    {"standard": "SOC2", "status": "Compliant",
                        "notes": "Logging active"},
                    {"standard": "PCI-DSS", "status": "Compliant" if is_banking else "Partially Compliant",
                        "notes": "Secure bindings active."}
                ],
        ...
        # D. Complexity Auditor Mock
            ...
            if is_crud and ("kubernetes" in lower_prompt or "aks" in lower_prompt or "k8s" in lower_prompt):
                ...
                warnings.append(
                    "Kubernetes container clusters selected for simple CRUD apps introduce excessive operational overhead.")
            if "service mesh" in lower_prompt:
                is_overengineered = True
                warnings.append(
                    "Service meshes are highly complex and likely unnecessary for this scale.")

            if not warnings:
                warnings.append(
                    "Basic networking is isolated within private subnet scopes.")
        ...
        # E. Cost Optimization Mock
            ...
            cost = 45.00
            breakdown = [
                {"service": "Core VM instances", "cost": 25.00,
                    "reason": "Standard compute instances"},
                {"service": "Relational DB storage",
                    "cost": 20.00, "reason": "Flexible DB engine"}
            ]

            if is_streaming or is_high_scale:
                cost = 420.00
                breakdown = [
                    {"service": "Secure Load Balancer / WAF",
                        "cost": 60.00, "reason": "API WAF ingress tier"},
                    {"service": "Active Container Replicas", "cost": 150.00,
                        "reason": "Scale container app engines"},
                    {"service": "PostgreSQL HA database", "cost": 125.00,
                        "reason": "Managed redundant transactional DB"},
                    {"service": "Redis Cache node", "cost": 45.00,
                        "reason": "In-memory caching"},
                    {"service": "Blob Storage CDN", "cost": 40.00,
                        "reason": "Static content delivery edge networks"}
                ]
