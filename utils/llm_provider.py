...
class OllamaProvider(AIProvider):
    def __init__(self):
        ...
        logger.info(
            f"Initialized OllamaProvider using model: {self.model} at {self.base_url}")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(f"{self.base_url}/api/generate", json=payload, timeout=300) as resp:
                    if resp.status != 200:
                        ...
                        raise Exception(
                            f"Ollama failed with status {resp.status}")
    ...
    def _clean_and_parse_json(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Failed to parse JSON from Ollama output: {e}\nRaw Output: {text}")
            raise
...
class ResilientProvider(AIProvider):
    def __init__(self, primary: AIProvider, secondary: Optional[AIProvider] = None, fallback: Optional[AIProvider] = None):
        ...
        self.last_active_provider = getattr(
            primary, "provider_name", "Resilient")
    ...
    async def generate_json(self, system_prompt: str, user_prompt: str, schema: Optional[Any] = None) -> Dict[str, Any]:
        try:
            self.last_active_provider = getattr(
                self.primary, "provider_name", "Primary")
            self.last_active_model = getattr(
                self.primary, "model_name", "unknown")
            self.last_fallback_trigger = "none"
            self._log_active(self.primary, "none")
            return await self.primary.generate_json(system_prompt, user_prompt, schema)
        except Exception as primary_error:
            primary_reason = self._classify_reason(
                primary_error, getattr(self.primary, "provider_name", "primary"))
            logger.warning(primary_reason)

            if self.secondary is not None:
                try:
                    self.last_active_provider = getattr(
                        self.secondary, "provider_name", "Secondary")
                    self.last_active_model = getattr(
                        self.secondary, "model_name", "unknown")
                    ...
                    secondary_reason = self._classify_reason(
                        secondary_error, getattr(self.secondary, "provider_name", "secondary"))
                    logger.warning(secondary_reason)
                    if self.fallback is not None:
                        self.last_active_provider = getattr(
                            self.fallback, "provider_name", "Mock")
                        self.last_active_model = getattr(
                            self.fallback, "model_name", "mock")
                        ...
                        raise RuntimeError(
                            f"All providers failed. Primary: {primary_reason}. Secondary: {secondary_reason}") from secondary_error
            ...
            if self.fallback is not None:
                ...
                raise RuntimeError(
                    f"Primary provider failed and no fallback configured. Error: {primary_reason}") from primary_error
...
def get_llm_provider() -> AIProvider:
    ...
    openai_available = bool(openai_key and not openai_key.startswith(
        "your_") and len(openai_key) > 5)
